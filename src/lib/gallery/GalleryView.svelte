<script lang="ts">
  import { cartStore, cartActions, commentCounts } from './stores'; // Add commentCounts
  ...
  function toggleCartItem(drawing: Drawing): void {
    console.log('toggleCartItem called with drawingId:', drawing.id); // Use drawing.id (UUID)
    ...
    <div class="action-buttons">
      ...
      <div class="button-row">
        ...
        >
          ...
          <span>Comments ({$commentCounts[drawing.id] || 0})</span>
        </button>
        
        <button 
          class="action-button cart-button {isInCart(drawing.id) ? 'in-cart' : ''}" 
        ...
        >
          ...
          <span>{isInCart(drawing.id) ? 'Remove from Cart' : 'Add to Cart'}</span>
        </button>
      </div>
    </div>
  </div>
