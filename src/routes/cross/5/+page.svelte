<script lang="ts">
  ...
  function generateRandomOperatorChain(start = 2, target = 18, attempts = 60000) {
    ...
    function runBatch() {
      ...
      if (i < attempts) {
        ...
        renderScene(coordinateChains, redHighlightChains, blueHighlightChains, greenHighlightChains, meanPath);
  ...
  function renderScene(data: number[][][], redLines: number[][][], blueLines: number[][][], greenLines: number[][][], meanPath: Map<number, number[]>) {
