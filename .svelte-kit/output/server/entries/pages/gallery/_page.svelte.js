...
function GalleryView($$payload, $$props) {
  ...
  $$payload.out += `<div class="gallery-item svelte-huzhr6"><div class="image-container svelte-huzhr6"><img${attr("src", placeholder)}${attr("data-src", drawing.image_data)}${attr("alt", `User drawing ${drawing.drawing_id}`)} class="gallery-image svelte-huzhr6" loading="lazy"> <button class="like-button-overlay svelte-huzhr6"><svg width="24" height="24" viewBox="0 0 24 24"${attr("fill", likedStates[drawing.drawing_id] ? "#ff4757" : "none")}${attr("stroke", likedStates[drawing.drawing_id] ? "#ff4757" : "#ffffff")} stroke-width="2"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"></path></svg> <span class="like-count svelte-huzhr6">${escape_html(store_get($$store_subs ??= {}, "$likesStore", likesStore)[drawing.drawing_id] || drawing.likes || 0)}</span></button></div> <div class="gallery-content svelte-huzhr6"><div class="drawing-info svelte-huzhr6"><h3 class="drawing-title svelte-huzhr6">${escape_html(drawing.drawing_id)}</h3> <p class="drawing-date svelte-huzhr6">Created: ${escape_html(new Date(drawing.created_at).toLocaleDateString())}</p></div> <div class="action-buttons svelte-huzhr6"><div class="button-row svelte-huzhr6"><button class="action-button preview-button svelte-huzhr6"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg> <span>Preview</span></button> <button class="action-button preview-3d svelte-huzhr6"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"></path><path d="M12 18a6 6 0 100-12 6 6 0 000 12z"></path><path d="M12 14a2 2 0 100-4 2 2 0 000 4z"></path></svg> <span>3D View</span></button></div> <div class="button-row svelte-huzhr6"><button class="action-button comments-button svelte-huzhr6"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"></path></svg> <span>Comments (${escape_html(drawing.comments?.length || 0)})</span></button> <button${attr_class(`action-button cart-button ${stringify(isInCart(drawing.drawing_id) ? "in-cart" : "")}`, "svelte-huzhr6")}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 20a1 1 0 100-2 1 1 0 000 2z"></path><path d="M20 20a1 1 0 100-2 1 1 0 000 2z"></path><path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 002-1.61L23 6H6"></path></svg> <span>${escape_html(isInCart(drawing.drawing_id) ? "Remove from Cart" : "Add to Cart")}</span></button></div></div></div></div>`;
...
function GalleryContainer($$payload, $$props) {
  ...
  async function fetchDrawings(page) {
    ...
    try {
      const { data, count, error: fetchError } = await supabase.from("drawings").select("id, drawing_id, image_data, likes, created_at, user_id, user_email, blocked, title", { count: "exact" }).eq("blocked", false).order("created_at", { ascending: false }).range(offset, offset + itemsPerPage - 1);
  ...
  function handlePreview(imageData) {
