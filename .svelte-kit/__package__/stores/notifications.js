...
export async function loadUnreadCount(userId) {
    try {
...
async function setupPushSubscription(userId) {
    ...
    try {
        ...
        if (existing)
            return;
        const token = await import('firebase/messaging').then(({ getToken }) => getToken(messaging, { vapidKey, serviceWorkerRegistration: registration }));
...
async function setupRealtimeSubscriptions(userId) {
    ...
    const { notifications } = getStoreSnapshot();
    const types = Object.keys(notifications).filter((key) => notifications[key]);
    if (!types.length)
        return;
    const filter = `user_id=eq.${userId},type=in.(${types.join(',')})`;
    const unsub = subscribeToTable('notifications', (payload) => {
...
export async function setupNotifications() {
    ...
    const hasNotifications = Object.values(profile).some((val) => typeof val === 'boolean' && val);
    if (hasNotifications) {
        await setupPushSubscription(user.id);
        await setupRealtimeSubscriptions(user.id);
    }
